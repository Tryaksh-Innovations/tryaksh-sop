...
async function load(
  workflowSlug: string,
  stageNumber: string
): Promise<StagePageData | null> {
  try {
    const wf = await getWorkflowBySlug(workflowSlug);
    if (!wf) return null;
...
export default async function HandbookStagePage({
  ...
  params: Promise<{ workflow: string; stageNumber: string }>;
}) {
  const { workflow: workflowSlug, stageNumber } = await params;
  const data = await load(workflowSlug, stageNumber);
  ...
  const { workflow, stage, items, prev, next } = data;
  if (!stage) notFound();
  ...
  return (
    <div className="space-y-10">
      ...
      <header className="border-t border-rule-3 pt-6">
        <div className="flex flex-wrap items-center gap-2 mb-3">
          <span className="font-mono text-[11px] text-ink-3 uppercase tracking-[0.12em]">
            {workflow.slug.toUpperCase()} · Stage {stage.stageNumber}
          </span>
          {stage.isLockGate && (
          ...
          {stage.isDecisionGate && (
            <Badge variant="warn">decision gate</Badge>
          )}
      ...
      <nav className="border-t border-rule-3 pt-5 grid grid-cols-2 gap-4">
        {prev ? (
          <Link
            href={`/handbook/${workflow.slug}/stages/${prev.stageNumber}`}
        ...
        {next && (
          <Link
            href={`/handbook/${workflow.slug}/stages/${next.stageNumber}`}
