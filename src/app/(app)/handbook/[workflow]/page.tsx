...
import { ChevronRight, Lock, BookOpen, ShieldCheck, ArrowLeft } from "lucide-react";
import {
  getWorkflowBySlug,
...
const DOC_ID_BY_SLUG: Record<string, string> = {
  pcb: "TRYAKSH-SOP-PCB-001",
  mech: "TRYAKSH-SOP-MECH-001",
};
...
> = {
  pcb: {
    titleLine1: "PCB Design",
    titleLine2: "Handbook.",
    blurb:
      "The Tryaksh ten-stage workflow with breadboard validation — read-only reference. CEO co-owns parts selection and schematic intent; designer owns execution; no PCB layout begins until the risky parts of the circuit have been physically validated.",
  },
  mech: {
    titleLine1: "Mechanical Design",
    titleLine2: "Handbook.",
    blurb:
      "The Tryaksh eleven-stage workflow with Datum Lock and First-Article Inspection. CEO co-owns concept and material decisions; designer owns execution; datums are locked before modeling begins; no part goes to manufacturing without a written DFM check and a formal first-article inspection on return.",
  },
};

export default async function HandbookWorkflowPage({
  params,
}: {
  ...
  const workflow = await getWorkflowBySlug(workflowSlug).catch(() => null);
  if (!workflow) notFound();

  const stages = await getStagesForWorkflow(workflow.id).catch(() => []);
  const diagram = DIAGRAM_BY_SLUG[workflow.slug];
  const docId = DOC_ID_BY_SLUG[workflow.slug] ?? `TRYAKSH-SOP-${workflow.slug.toUpperCase()}-001`;
  const copy = COPY_BY_SLUG[workflow.slug] ?? {
    titleLine1: workflow.name,
    titleLine2: "Handbook.",
        ...
        <section>
          <div className="flex items-baseline justify-between mb-3">
            <div>
              ...
              <p className="mono-caps text-ink-3 mt-1">Plate 01 · One-pager</p>
            </div>
        ...
        <div className="flex items-baseline justify-between mb-4">
          <div>
            ...
            <p className="mono-caps text-ink-3 mt-1">
              {stages.length} stages · sequentially gated
            </p>
          </div>
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {stage.isDecisionGate && (
                      <Badge variant="warn">decision</Badge>
                    )}
